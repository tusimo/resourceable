...
use Tusimo\Resource\Model\ModelNotFoundException;
...
class BelongsToMany extends Relation
{
    ...
    /**
     * The pivot model for the relation.
     *
     * @var Model
     */
    protected Builder $pivotBuilder;
    ...
    public function wherePivot($column, $operator = null, $value = null)
    {
        ...
        return $this->pivotBuilder->where($column, $operator, $value);
     ...
     * @return $this
     */
    public function wherePivotIn($column, $values)
    {
        $this->pivotWhereIns[] = func_get_args();
        return $this->pivotBuilder->whereIn($column, $values);
     ...
     * @param array $columns
     * @param mixed $id
     * @return \Hyperf\Utils\Collection|\Tusimo\Resource\Model\Model
     */
     ...
     * @param array $columns
     * @param mixed $id
     * @return null|\Tusimo\Resource\Model\Collection|\Tusimo\Resource\Model\Model
     */
     ...
     * @param array $columns
     * @param mixed $id
     * @return \Tusimo\Resource\Model\Collection|\Tusimo\Resource\Model\Model
    ...
    public function get($columns = ['*'])
    {
        ...
        $columns = $builder->getQuery()->getQuerySelect()->getSelects() ? [] : $columns;

        // first get pivot models
        $pivots = $this->pivotBuilder->get();
        ...
        $models = $builder
            ->whereIn($this->getRelatedKeyName(), $this->getRelatedPivotKeys($pivots))
            ->getModels();

        $models = $this->hydratePivotRelations($pivots, $models);
    ...
    protected function hydratePivotRelations(Collection $pivots, $models)
    {
        $models = collect($models)->keyBy($this->getRelatedKeyName());
        $newModels = [];
        foreach ($pivots as $pivot) {
            /**
             * @var Model $model
             */
            $model = $models[$pivot->{$this->getRelatedPivotKeyName()}];
            $model = $model->duplicate()->setAttribute(
                "{$this->accessor}_{$this->getForeignPivotKeyName()}",
                $pivot->{$this->getForeignPivotKeyName()}
            );
            $model = $model->duplicate()->setAttribute(
                "{$this->accessor}_{$this->getRelatedPivotKeyName()}",
                $pivot->{$this->getRelatedPivotKeyName()}
            );
            $newModels[] = $model;
        }
        return $newModels;
    ...
    protected function getRelatedPivotKeys(Collection $pivots): array
    {
        return $pivots->pluck($this->getRelatedPivotKeyName())
            ->toArray();
    }

    protected function getForeignPivotKeys(Collection $pivots): array
    {
        return $pivots->pluck($this->getForeignPivotKeyName())
            ->toArray();
    }
     ...
     *
     * @param null|\Tusimo\Resource\Model\Builder $query
     * @return $this
     */
    ...
    {
        $this->pivotBuilder->where(
            $this->getForeignPivotKeyName(),
            '=',
